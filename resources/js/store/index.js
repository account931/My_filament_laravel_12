...
//Vuex store changed to Pinia store!!!
...
export const useOwnerStore = defineStore('myStore', {  //myStore' is the unique ID of this store. Used internally by Pinia and dev tools
  state: () => ({
    count: 0,
    name: 'Pinia Store',
    posts: [],
  ...
  getters: {
    getCart: (state) => state.passport_api_tokenY
  },
  ...
  actions: {
    async getAllPosts() {
      ...
      try {
        ...
        if (response.data) {
          this.posts = response.data.data; //as posts are wrapped in data
          console.log(this.posts);
          Swal.fire("Done", "Articles are loaded (axios) (Pinia store).", "success");
        }
      } catch (err) {
        this.showLoader = false;
        ...
        if (err?.response?.status === 401 || err.message.includes("401")) {
          Swal.fire("Unauthenticated", "Check Bearer Token", "error");
          this.logout();
        }
      }
    },
...
export default new Vuex.Store({
    ...
    */
	...
	/*
    |--------------------------------------------------------------------------
    | Mutation section  NOT USED IN PIANIA
    |--------------------------------------------------------------------------
    |
    |
    */
