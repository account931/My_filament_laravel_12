    ...
    export default {
	   name: 'MapboxMap',
        ...
        mounted() { 
            ...
            //mapboxgl.accessToken = import.meta.env.VITE_MAPBOX_API_KEY; //reads Mapbox directly from .env, works fine on local, but fails on render.com  //window.mapbox_api_token_from_config.tokenMapBox;  //env('MAPBOX_API_KEY');  // Replace with your actual Mapbox access token
            mapboxgl.accessToken = window.MAPBOX_API_KEY;  //fix for render.com, gets it from from blade as => window.MAPBOX_API_KEY = "{{ config('services.mapbox.token') }}";
			...
			getAllVenuesCoordinates() {
               ...
               //Axios method http variant 
                ...
                .then(dataZ => {
                    ...
                    if (dataZ.data){ 
						...
						for (const key in dataZ.data.data) {
						 ...
						 //Laravel 12 fix, change lon to lng 
			            ...
			            for(let i=0; i < that.allVenuesCoords.length; i++){
						    ...
						    console.log(that.allVenuesCoords[i].coords.lng);  //lon  //Laravel 12 fix
 			                new mapboxgl.Marker()
                                .setLngLat([ that.allVenuesCoords[i].coords.lng, that.allVenuesCoords[i].coords.lat ])  // Set marker position
			                    .setPopup(
                                    new mapboxgl.Popup({ offset: 25 }) // Optional offset for styling
	            ...
	            .catch(function(err){ 
                    ...
                    Swal.fire("Crashed", "You are in catch, check" + " <b> mapboxgl.accessToken</b>", "error");
                    ...
                    if(err == "Error: Request failed with status code 401" ||  err == "Unauthenticated."){ //if Rest endpoint returns any predefined error
                        ...
                        Swal.fire("Unauthenticated2", "Check Bearer Token2", "error");
